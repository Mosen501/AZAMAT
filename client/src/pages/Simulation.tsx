...
import { Clock, ChevronRight, AlertOctagon } from "lucide-react";
...
export default function Simulation() {
  ...
  useEffect(() => {
    ...
    if (state.level !== "beginner" || !state.role || !state.scenarioId) {
    ...
    return (
      <Layout className="items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-display uppercase text-primary animate-pulse tracking-widest text-lg">
            {text("Loading Scenario...", "جار تحميل السيناريو...")}
          </p>
        </div>
      </Layout>
    );
      ...
      <Layout className="items-center justify-center">
        <div className="glass-panel p-8 text-center max-w-lg">
          ...
          <h2 className="text-2xl font-display text-destructive mb-2 uppercase">
            {text("System Error", "خطأ في النظام")}
          </h2>
          <p className="text-muted-foreground mb-6">
            {text(
              "Failed to load simulation parameters. Please contact administration.",
              "تعذر تحميل إعدادات المحاكاة. يرجى التواصل مع الإدارة.",
            )}
          </p>
          <CyberButton onClick={() => window.location.reload()}>
            {text("Reload", "إعادة التحميل")}
          </CyberButton>
        </div>
      </Layout>
    );
  ...
  const displayScenario = localizeScenario(scenario, isArabic);
  const currentStep = currentStepId ? displayScenario.steps[currentStepId] : null;
  ...
  const handleChoice = (choiceId: string) => {
    ...
    recordChoice(currentStep.id, choiceId, choice.scoreDeltas);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="font-display text-lg uppercase tracking-wider text-muted-foreground">
              {text("Incoming Intelligence", "المعطيات الواردة")}
            </h2>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="mb-3 text-sm font-display uppercase tracking-wider text-primary/80">
        ...
        <div className="flex flex-col gap-4 h-full">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="font-display text-lg uppercase tracking-wider text-muted-foreground">
              {text("Awaiting Directive", "بانتظار التوجيه")}
            </h2>
          </div>
              ...
              >
                {currentStep.choices.map((choice, index) => (
                  <motion.button
                    ...
                    className={`flex-1 bg-secondary/30 border border-primary/20 hover:border-primary hover:bg-primary/10 p-5 clip-chamfer-sm transition-all duration-300 group hover:shadow-[0_0_15px_rgba(0,240,255,0.15)] flex items-center ${
                      isArabic ? "text-right justify-end" : "text-left justify-start"
                    }`}
                  >
                    <span className="text-foreground/90 group-hover:text-white font-medium leading-snug">
                      {choice.text}
                    </span>
                  </motion.button>
                ))}
